use poise::serenity_prelude::{ self as serenity, ComponentInteraction, CreateAttachment, CreateEmbed, CreateInteractionResponseMessage, CreateMessage, EditInteractionResponse, EditMessage};
...
impl ScoreMapping {
    async fn new(ctx: &serenity::Context, contents: [&str; 4]) -> ScoreMapping {
        let user_id: u64 = contents[2].parse().expect("not a u64");
    ...
    let _ = match identifier {
        "approve_with_upload" => {
            ...
            let title = match score.score_type {
                ScoreType::ScoreId => upload_score_by_score(ctx, component, &score).await?,
                ScoreType::ReplayFile => upload_score_by_replay(ctx, component, &score).await?,
            };
            if title.is_some() {
        ...
        "approve_no_upload" => {
            let score = ScoreMapping::new(ctx, data.try_into().expect("Data must have 3 values")).await;
            let title = match score.score_type {
                ScoreType::ScoreId => get_score_metadata_by_score(ctx, component, &score).await?,
                ScoreType::ReplayFile => get_score_metadata_by_replay(ctx, component, &score).await?,
            };
            if title.is_some() {
                score.requesting_user.dm(ctx, CreateMessage::default().add_embed(embeds::suggestion_approved_embed(&title.unwrap())?)).await?;
                message.edit(ctx, EditMessage::default().components(vec![])).await?;
...
async fn get_score_metadata_by_replay(ctx: &serenity::Context, component: &serenity::ComponentInteraction, score: &ScoreMapping) -> Result<Option<String>, Error> {
    ...
    component.edit_response(ctx, EditInteractionResponse::default()
        .content(format!("```{}``````{}```", title, description))
        .new_attachment(CreateAttachment::file(&replay_file, "replay.osr").await?)
        .new_attachment(CreateAttachment::bytes(thumbnail, "thumbnail.jpeg"))
    ).await?;
...
async fn get_score_metadata_by_score(ctx: &serenity::Context, component: &serenity::ComponentInteraction, score: &ScoreMapping) -> Result<Option<String>, Error> {
    ...
    let map = osu::get_osu_instance().beatmap().map_id(score.map_id.clone()).await?;
